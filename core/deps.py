...
async def get_current_user(db: AsyncSession = Depends(get_session), token: str= Depends(oauth2_schema)) -> usuario_model:
    ...
    async with db as session:
        query = select(usuario_model).filter(
            usuario_model.id == int(token_data.username)
        )
